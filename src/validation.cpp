        ...
        bool in_memory)
{
    ...
    // Only the snapshot chainstate needs candidates populated here. The
    // historical chainstate's candidates remain valid because LoadChainTip
    // above took the early return (the chain tip was already set by
    // PopulateAndValidateSnapshot), so no nSequenceId values were modified.
