...
/*
    ...
    while (!HasValidProofOfWork(headers, Params().GetConsensus())) {
            ++(headers[0].nNonce);
    }
  ...
  */  
    ...
    while (!FuzzCheckPoW(header.GetHash(), header.nBits, header.nNonce, Params().GetConsensus())) {
        ++(header.nNonce);
    }
    ...
    LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 100)
    {
        ...
        CallOneOf(
            ...
            [&]() NO_THREAD_SAFETY_ANALYSIS {
                // Send a block
                auto block = finalized_block();
                //std::cout << block.m_checked_merkle_root << std::endl;
