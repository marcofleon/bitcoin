...
FUZZ_TARGET(block_index_diff, .init = init)
{
    ...
    auto block_index_store = BlockTreeStore(block_store_dir, ::Params(), true);
    ...
    for (int i = 0; i < blocks_count; i++) {
        ...
        //std::string strblockhash = current_block_hash.ToString();
        //printf("fuzz test hash = %s\n", strblockhash.c_str());
    ...
    reindexing_store = block_index_store.ReadReindexing();
