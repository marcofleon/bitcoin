...
BOOST_AUTO_TEST_CASE(AddToSetCollisionTest)
{
    ...
    // Collisions cannot be checked if the peer is not fully registered
    BOOST_CHECK(!tracker.HasCollision(peer_id0, wtxid, collision, short_id));
    ...
    // Checking for the collision externally should also work
    BOOST_CHECK(tracker.HasCollision(peer_id0, wtxid, collision, short_id));
}
