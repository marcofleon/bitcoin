...
static bool GenerateBlock(ChainstateManager& chainman, CBlock& block, uint64_t& max_tries, std::shared_ptr<const CBlock>& block_out, bool process_new_block)
{
    ...
    while (max_tries > 0 && block.nNonce < std::numeric_limits<uint32_t>::max() && !FuzzCheckPoW(block.GetHash(), block.nBits, block.nNonce, chainman.GetConsensus()) && !chainman.m_interrupt) {
        ++block.nNonce;
        --max_tries;
    }
