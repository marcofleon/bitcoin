...
BlockTreeStore::BlockTreeStore(const fs::path& path, const CChainParams& params, bool wipe_data)
    : m_block_tree_store_dir{path},
      m_params{params}
{
    fs::create_directories(m_block_tree_store_dir);
    if (wipe_data) {
        fs::remove(m_block_tree_store_dir / HEADER_FILE_NAME);
        fs::remove(m_block_tree_store_dir / BLOCK_FILES_FILE_NAME);
    }
    bool header_file_exists{fs::exists(m_block_tree_store_dir / HEADER_FILE_NAME)};
    bool block_files_file_exists{fs::exists(m_block_tree_store_dir / BLOCK_FILES_FILE_NAME)};
    if (header_file_exists ^ block_files_file_exists) {
        throw BlockTreeStoreError("Block tree store is an inconsistent state");
    }
    if (!header_file_exists && !block_files_file_exists) {
        CreateHeaderFile();
...
{
    {
        FILE* file = fsbridge::fopen(m_block_tree_store_dir / HEADER_FILE_NAME, "wb");
        if (!file) {
            throw BlockTreeStoreError(strprintf("Unable to open file %s\n", fs::PathToString(m_block_tree_store_dir)));
        }
    ...
    auto file{AutoFile{fsbridge::fopen(m_block_tree_store_dir / HEADER_FILE_NAME, "rb+")}};
...
bool BlockTreeStore::ReadReindexing() const
{
    auto file{AutoFile{fsbridge::fopen(m_block_tree_store_dir / HEADER_FILE_NAME, "rb")}};
    file.seek(HEADER_FILE_REINDEX_FLAG_POS, SEEK_SET);
    bool reindexing;
...
{
    {
        FILE* file = fsbridge::fopen(m_block_tree_store_dir / BLOCK_FILES_FILE_NAME, "wb");
        if (!file) {
            throw BlockTreeStoreError(strprintf("Unable to open file %s\n", fs::PathToString(m_block_tree_store_dir)));
        }
        AllocateFileRange(file, BLOCK_FILES_DATA_START_POS, BLOCK_FILES_FILE_SIZE);
        auto autofile{AutoFile{file}};
        autofile.Commit();
    }

    auto file{AutoFile{fsbridge::fopen(m_block_tree_store_dir / BLOCK_FILES_FILE_NAME, "rb+")}};
...
bool BlockTreeStore::ReadPruned() const
{
    auto file{AutoFile{fsbridge::fopen(m_block_tree_store_dir / BLOCK_FILES_FILE_NAME, "rb")}};
    file.seek(BLOCK_FILES_PRUNE_FLAG_POS, SEEK_SET);
    bool pruned;
...
bool BlockTreeStore::WriteBatchSync(const std::vector<std::pair<int, CBlockFileInfo*>>& fileInfo, int32_t last_file, const std::vector<CBlockIndex*>& blockinfo)
{
    ...
    {
        auto block_files_file{AutoFile{fsbridge::fopen(m_block_tree_store_dir / BLOCK_FILES_FILE_NAME, "rb+")}};
    ...
    {
        auto header_file{AutoFile{fsbridge::fopen(m_block_tree_store_dir / HEADER_FILE_NAME, "rb")}};
    ...
    // Write the header data
    {
        ...
        for (CBlockIndex* bi : blockinfo) {
            ...
            //printf("Write [Store]: Writing data for block with intended hash = %s\n", bi->GetBlockHash().ToString().c_str());
    ...
    const util::SignalInterrupt& interrupt)
{
    ...
    while (file.tell() < data_end_pos) {
        ...
        file >> diskindex;
        //printf("Load [store]: Read record, calculated hash = %s\n", diskindex.ConstructBlockHash().ToString().c_str());
