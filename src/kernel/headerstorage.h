...
inline constexpr uint64_t HEADER_FILE_SIZE{0x1000000 * 2}; // 32 MiB
...
inline constexpr uint64_t BLOCK_FILES_FILE_SIZE{0x10000 * 2}; // 8 KiB
...
private:
    ...
    fs::path m_block_tree_store_dir;
...
public:
    ...
    bool ReadReindexing() const;
    void WriteReindexing(bool reindexing) const;

    void ReadLastBlockFile(int32_t& last_block) const;
    bool ReadPruned() const;
    void WritePruned(bool pruned) const;

    bool WriteBatchSync(const std::vector<std::pair<int, CBlockFileInfo*>>& fileInfo, int32_t last_file, const std::vector<CBlockIndex*>& blockinfo)
